...
'Summary Headers:
         'Total Data Summary Table Header
        Range("N1:P1").Merge
        Range("N1:P1") = "Total DATA Summary:"
        Range("N1:P1").Font.Bold = True
        Range("N1:P1").Font.ColorIndex = 1
    ...
    'Items in "Ticker" Column: Ticker Names listed aphabetically
        ...
        'Table Header
        Range("S3") = "SHEET COUNT SUMMARY"
        Range("S3").Font.Bold = True
        Range("S3").Font.ColorIndex = 1
        Range("S3:T3").Merge
    ...
    Next ws
        'Copy Data from Last Sheet (source) to  First Sheet (destination)
        Sheets(Sheets.Count).Range("R4:T7").Copy Destination:=Worksheets(1).Range("N2")
